...
class EnvelopeLotProcessor(EnvelopeProcessor):
    ...
    def _process_internal(
        ...
        sorted_envelopes = sorted(
            envelopes,
            key=lambda e: (e.date, e.original_order or 0)
        )
        ...
        while envelope_idx < len(sorted_envelopes) or action_idx < len(self.corporate_actions):
            ...
            if process_envelope:
                ...
                try:
                    ...
                    logger.error(f"Unexpected error processing {envelope.envelope_id}: {e}")
                    warnings.append(LotTrackingWarning(
                        processor_name=self.processor_name,
                        severity='ERROR',
    ...
    def get_lot_registry(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'stats': {
                ...
                'movement_types': {
                    'creates': sum(1 for m in self.lot_movements if m['action'] == 'CREATE'),
                    'consumes': sum(1 for m in self.lot_movements if m['action'] == 'CONSUME'),
                    'transfers': sum(1 for m in self.lot_movements if m['action'] == 'TRANSFER')
                }
            }
        }
    ...
    def _handle_buy(self, envelope: Envelope) -> Envelope:
        ...
        #
        # Note: Commission/fees are already included in the cash outflow amount.
        # For example, if you buy shares for £20000 + £7.99 commission:
        #   Assets:Broker          X SHARES
        #   Expenses:Commission    7.99 GBP      (for expense tracking)
        #   Assets:Cash          -20007.99 GBP   (total including commission)
        #
        # The cost basis is 20007.99 / X, NOT (20007.99 + 7.99) / X
        # The commission expense posting is for P&L tracking, not cost calculation.
        if envelope.unit_price:
            ...
            # Cash outflow already includes any commissions/fees
        ...
        self.lot_movements.append({
            ...
            'quantity': str(envelope.inbound_units),
            'cost_per_unit': str(cost_per_unit),
            'narration': envelope.narration or f'Purchase of {envelope.inbound_type}'
        })
        ...
        if not envelope.unit_price and cost_per_unit != Decimal('0'):
            ...
            envelope = enhance_envelope(
                envelope,
                reason="Added lot creation info",
                metadata=updated_metadata
            )
    ...
    def _handle_sell(self, envelope: Envelope, warnings: List[LotTrackingWarning]) -> Envelope:
        """
        ...
        for lot in self.lots[key]:
            ...
            self.lot_movements.append({
                ...
                'cost_per_unit': str(lot['cost_per_unit']),
                'narration': envelope.narration or f'Sale of {envelope.outbound_type}'
            })
        ...
        if consumed_lots:
            ...
            envelope = enhance_envelope(
                envelope,
                reason="Added consumed lot info",
                metadata=updated_metadata
            )
    ...
    def _handle_transfer(self, envelope: Envelope, warnings: List[LotTrackingWarning]) -> Envelope:
        """
        ...
        from cassoulet.utils.envelope_utilities import get_outbound_account, get_inbound_account
        ...
        if transferred_lots:
            ...
            envelope = enhance_envelope(
                envelope,
                reason="Added transfer lot info",
                metadata=updated_metadata
            )
    ...
    def _handle_stock_split(self, split: StockSplit, warnings: List[LotTrackingWarning]) -> CorporateActionResult:
        """
        ...
        logger.info(f"Transforming {len(self.lots[key])} lots for {split.commodity} split ({split.ratio}:1)")
